import React from "react";

export default function Faqminusicon() {
  return (
    <svg
      width="100%"
      height="100%"
      viewBox="0 0 52 52"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <rect x="0.5" y="0.5" width="51" height="51" rx="25.5" fill="#1F1F1F" />
      <rect x="0.5" y="0.5" width="51" height="51" rx="25.5" stroke="#333333" />
      <path
        d="M33 25H19C18.7348 25 18.4804 25.1054 18.2929 25.2929C18.1054 25.4804 18 25.7348 18 26C18 26.2652 18.1054 26.5196 18.2929 26.7071C18.4804 26.8946 18.7348 27 19 27H33C33.2652 27 33.5196 26.8946 33.7071 26.7071C33.8946 26.5196 34 26.2652 34 26C34 25.7348 33.8946 25.4804 33.7071 25.2929C33.5196 25.1054 33.2652 25 33 25Z"
        fill="#E7BEB1"
      />
    </svg>
  );
}
